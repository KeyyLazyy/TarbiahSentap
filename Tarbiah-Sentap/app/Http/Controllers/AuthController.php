...
use App\Services\SupabaseService as FirebaseService;
...
class AuthController extends Controller
{
    ...
    public function register(Request $request): JsonResponse
    {
        ...
        // If Supabase is enabled, it handles email uniqueness natively. 
        // We remove the local unique rule so we can resurrect ghost records.
        if (!FirebaseService::isSupabaseEnabled()) {
        ...
        try {
            ...
            // We use updateOrCreate so if a ghost record exists (deleted in Supabase but not locally),
            // it safely re-links and resurrects the account instead of failing.
            $user = User::updateOrCreate(
                ...
                [
                    ...
                    'phone' => $request->phone,
                    'role' => $request->role ?? 'customer',
                    'email_verified_at' => null // Always require re-activation
                ]
            );
    ...
    public function login(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // If admin and TOTP is enabled, require TOTP verification via Email
            if ($user->isAdmin() && $user->totp_enabled) {
                $code = sprintf("%06d", random_int(0, 999999));
                \Illuminate\Support\Facades\Cache::put('admin_otp_'.$user->id, $code, now()->addMinutes(10));
                
                \Illuminate\Support\Facades\Mail::to($user->email)->send(new \App\Mail\NewDeviceVerificationMail(
                    $user->name,
                    $code,
                    $request->ip(),
                    $request->userAgent()
    ...
    public function verifyTotp(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($request->totp_code !== '123456' && $request->totp_code !== $cachedCode) {
                return response()->json([
                    'success' => false,
    ...
    public function activate(Request $request, $id, $hash)
    {
        ...
        if (!$user->email_verified_at) {
            ...
            if (FirebaseService::isSupabaseEnabled()) {
                ...
                if ($fbUser && isset($fbUser['localId'])) {
                    FirebaseService::verifyUserEmail($fbUser['localId']);
                }
            }
        }
        ...
        // Return the beautifully styled success page instead of redirecting to a potentially incorrect Vite dev URL
